...
void resize_image(const cv::Mat &source_image, cv::Mat &resized_image, Options &options) {
  ...
  auto max_size = options.detection_max_size;
  ...
  if (resize_height % 32 == 0) {
    ...
    resize_height = static_cast<int>(ceil(static_cast<double>(resize_height) / 32) * 32);
  }
  if (resize_width % 32 == 0) {
    ...
    resize_width = static_cast<int>(ceil(static_cast<double>(resize_width) / 32) * 32);
  }
