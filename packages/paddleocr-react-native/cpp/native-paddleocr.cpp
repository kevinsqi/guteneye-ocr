...
NativePaddleOcr::NativePaddleOcr(std::unordered_map<std::string, std::any> rawOptions, const std::string &assetDir,
...
std::vector<TextLine> NativePaddleOcr::detect(std::string &image_path) {
  Timer timer;
  timer.start();
  ...
  if (m_options.is_debug) {
    ...
    std::cout << "[DEBUG] Recognition costs " << (int)recognition_total_sum << "ms" << std::endl;
