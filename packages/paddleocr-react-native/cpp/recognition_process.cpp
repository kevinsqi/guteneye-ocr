...
ImageRaw RecognitionPredictor::preprocess(const cv::Mat &source_image, cv::Mat &resized_image) {
  ...
  // PP-OCRv5 uses height=48, max_width=480
  int rec_height = m_options.recognition_image_height;
  int rec_max_width = rec_height * 10;  // 480 for height 48

  resized_image = recognition_resize_image(source_image, rec_height, rec_max_width, m_options);
